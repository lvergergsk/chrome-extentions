...
- `utils` - 个人工具箱。包含：
  ...
  - HoYoLAB 自动签到：每天 09:10 和 15:10 检查原神、崩坏：星穹铁道、绝区零，Chrome 启动时也会补查一次；直接复用当前 Chrome profile 的 HoYoLAB 登录状态，不保存 Cookie。
...
5. 在同一个 Chrome profile 登录一次 HoYoLAB；之后由扩展后台签到，不需要 Windows 计划任务。
